...
public class Calculate {
	...
	//Part 3
	public static double exponent(double x, int n) { // Returns the exponent of the inputed numbers, pos nums only 
		if(x==0 && n==0)
			throw new java.lang.ArithmeticException("0 to the power of 0 is undefined");
		if(n==0)
			return 1;
		if(x==0)
			return 0;
	...
	public static String quadForm(int a, int b, int c) {  // Finds the roots of a quadratic equation
		...
		return (round2(Calculate.min(formPlus, formMinus)) + " and " + round2(Calculate.max(formPlus, formMinus))); // return the rounded min and max roots
	}
